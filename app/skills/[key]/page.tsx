...
import { getSkillByKey, getSkillVersions } from "@/lib/capabilities/queries";
import { ProtectedNav } from "@/components/protected-nav";
import { requireUser } from "@/lib/auth/require-user";
import { formatDate } from "@/lib/projects/format";

export const metadata: Metadata = {
  title: "Skill | MintChipOS",
};
...
export default async function SkillDetailPage({
  ...
  const currentVersion = versions.find(
    (version) => version.id === skill.current_version_id,
  );
                ...
                <div>
                  ...
                  <p className="mt-1 text-zinc-900 dark:text-zinc-100">
                    {currentVersion?.source_type
                      ? formatSkillSourceType(currentVersion.source_type)
                      : "—"}
                  </p>
                </div>
                <div>
                  ...
                  <p className="mt-1 text-zinc-900 dark:text-zinc-100">
                    {currentVersion?.source_reference ?? "—"}
                  </p>
                </div>
              </div>
              {currentVersion?.metadata ? (
                <div className="mt-5">
                  ...
                  <pre className="mt-2 overflow-x-auto rounded-lg bg-zinc-50 p-3 text-xs leading-5 text-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
                    {formatJson(currentVersion.metadata)}
                  </pre>
                </div>
              ) : null}
            </div>
